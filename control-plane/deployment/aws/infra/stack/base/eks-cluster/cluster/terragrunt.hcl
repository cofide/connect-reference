...
dependency "vpc" {
  ...
  # provided directly in common.local.hcl (e.g. deploying into an existing VPC).
...
locals {
  default_config = {
    cluster_name    = "cofide-connect-control-plane-aws-reference-arch"
    cluster_version = "1.35"

    cluster_role_name = "cofide-connect-control-plane-aws-reference-arch-cluster"
    node_role_name    = "cofide-connect-control-plane-aws-reference-arch-node"
    cluster_sg_name   = "cofide-connect-control-plane-aws-reference-arch-cluster"
    node_sg_name      = "cofide-connect-control-plane-aws-reference-arch-nodes"
    kms_key_alias     = "alias/cofide-connect-control-plane-aws-reference-arch-eks-secrets"
    ...
    node_groups = {
      workers = {
        name     = "cofide-connect-control-plane-aws-reference-arch-workers"
        ami_type = "AL2023_ARM_64_STANDARD"
        # m7g.xlarge: 4 vCPU / 16 GB RAM. Graviton3 offers better price/performance
        # than equivalent x86_64 instances and double the RAM of c6i.xlarge at a
        # lower cost. Requires ARM64 container images — all components in this
        # reference architecture support ARM64.
...
inputs = {
  ...
  # networking unit. Set vpc_id and private_subnet_ids in common.local.hcl to
  # deploy into an existing VPC without the networking unit dependency.
