...
locals {
  ...
  use_secrets_manager = !local.use_direct_creds && !var.aws_rds_iam_auth && var.db_actual_host == null
...
data "aws_secretsmanager_secret_version" "master" {
  count     = local.use_secrets_manager ? 1 : 0
  secret_id = data.aws_db_instance.this[0].master_user_secret[0].secret_arn
}
...
provider "postgresql" {
  ...
  port             = var.db_port
  database         = "postgres"
  username         = local.use_direct_creds ? var.db_username : (local.use_secrets_manager ? jsondecode(data.aws_secretsmanager_secret_version.master[0].secret_string)["username"] : var.db_admin_username)
  password         = local.use_direct_creds ? var.db_password : (local.use_secrets_manager ? jsondecode(data.aws_secretsmanager_secret_version.master[0].secret_string)["password"] : (local.use_external_iam ? data.external.iam_auth_token[0].result["token"] : null))
