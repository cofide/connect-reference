...
variable "zone_id" {
  description = "ID of the Route53 hosted zone that external-dns will manage"
  type        = string
}
...
variable "policy_name" {
  description = "Name of the IAM policy for external-dns"
  type        = string
}

variable "iam_mode" {
  description = "IAM authentication mode for external-dns. 'pod_identity' uses EKS Pod Identity; 'irsa' uses IAM Roles for Service Accounts."
  type        = string
...
variable "namespace" {
  description = "Kubernetes namespace that external-dns runs in"
  type        = string
