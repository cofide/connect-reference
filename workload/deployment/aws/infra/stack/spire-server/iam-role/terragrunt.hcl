...
locals {
  default_config = {
    role_name            = "cofide-connect-trust-zone-aws-reference-arch-spire-server"
...
inputs = {
  ...
  # db_resource_id and db_username are intentionally omitted. The trust zone
  # SPIRE server uses Connect as its datastore rather than RDS, so no
  # rds-db:connect permission is needed.
}
