...
Terragrunt unit that provisions the EKS cluster for a Cofide Connect trust zone. Creates:
...
By default the cluster is deployed into the same VPC and uses the same SSM jump instance as the Connect control-plane stack. Set `vpc_id`, `private_subnet_ids`, and `jump_security_group_id` in `common.local.hcl` to use a different VPC or jump host.
...
The API server has no public endpoint. Access from developer machines is via SSM port forwarding through the control-plane jump instance — no public IPs, no open inbound ports, no SSH keys.
...
JUMP_INSTANCE_ID=$(cd ../../../../../../../control-plane/deployment/aws/infra/stack/base/jump && terragrunt output -raw instance_id)
```
